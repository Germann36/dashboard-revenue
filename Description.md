...
- Основным источником данных является выгрузка из билетной системы в csv, загружаемая напрямую в DataLens. В ближайшей перспективе источник данных будет загружаться в облачное хранилище Object Storage и подключаться к BI-системе через Yandex Query
